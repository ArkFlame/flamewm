...
// FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-CHROME-01)
#include "flamewm/core/runtime.h"
#include "flamewm/core/scalemanager.h"
#include "flamewm/ui/metrics.h"
// FLAMEWM-UPSTREAM-HOOK-END(FW-CHROME-01)
#endif
#include "ywindow.h"
...
int YFrameWindow::borderXN() const {
    int base = ((frameDecors() & fdBorder) && !(hideBordersMaximized && isMaximizedFully()))
        ? ((frameDecors() & fdResize) ? wsBorderX : wsDlgBorderX) : 0;
    if (base == 0) return 0;
#ifdef FLAMEWM_PRODUCT_BUILD
    if (flamewm::Runtime::instance() && flamewm::Runtime::instance()->initialized() && flamewm::Runtime::instance()->scaleManager()) {
        int screen = desktop ? desktop->getScreenForRect(x(), y(), width(), height()) : 0;
        if (screen < 0) screen = 0;
        std::string conn = "screen-" + std::to_string(screen);
        flamewm::OutputId oid = flamewm::ScaleManager::makeOutputId(conn, "");
        int scale = flamewm::Runtime::instance()->scaleManager()->scaleFor(oid);
        flamewm::FlameMetrics m = flamewm::FlameMetrics::defaults().scaled(scale);
        // visualBorder is logical 1 scaled; blend with wsBorderX ratio to avoid global mutation
        // Use FlameMetrics visualBorder scaled as floor, but preserve theme border when larger.
        int scaledBorder = m.visualBorder;
        if (scale != 100) {
            // scale wsBorderX logically: (base*scale+50)/100
            scaledBorder = (base * scale + 50) / 100;
        }
        return scaledBorder;
    }
#endif
...
int YFrameWindow::borderYN() const {
    ...
    if (flamewm::Runtime::instance() && flamewm::Runtime::instance()->initialized() && flamewm::Runtime::instance()->scaleManager()) {
        ...
        flamewm::OutputId oid = flamewm::ScaleManager::makeOutputId(conn, "");
        int scale = flamewm::Runtime::instance()->scaleManager()->scaleFor(oid);
        int scaledBorder = (base * scale + 50) / 100;
        return scaledBorder;
    }
#endif
    return base;
...
int YFrameWindow::titleYN() const {
    if (hideTitleBarWhenMaximized && isMaximizedVert())
        return 0;
    if ((frameDecors() & fdTitleBar) == 0) return 0;
#ifdef FLAMEWM_PRODUCT_BUILD
    // FW-CHROME-01: per-output logical metrics via ScaleManager + FlameMetrics, no global mutation
    if (flamewm::Runtime::instance() && flamewm::Runtime::instance()->initialized() && flamewm::Runtime::instance()->scaleManager()) {
        int screen = -1;
        if (desktop) screen = desktop->getScreenForRect(x(), y(), width(), height());
        if (screen < 0) screen = 0;
        std::string conn = "screen-" + std::to_string(screen);
        flamewm::OutputId oid = flamewm::ScaleManager::makeOutputId(conn, "");
        int scale = flamewm::Runtime::instance()->scaleManager()->scaleFor(oid);
        flamewm::FlameMetrics m = flamewm::FlameMetrics::defaults().scaled(scale);
        return m.titlebarSize;
    }
#endif
    return wsTitleBar;
}
