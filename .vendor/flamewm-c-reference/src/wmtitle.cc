...
void YFrameTitleBar::paint(Graphics &g, const YRect &/*r*/) {
    ...
    // FlameWM FW-CHROME-01: whole-titlebar center with collision clamps.
    // Desired = (barWidth - total)/2, safe = [onLeft+pad, onRight-total-pad], clamp(desired,min,max)
    int stringOffset;
    {
        int total = tlen + llen + rlen;
        int desired = (int(width()) - total) / 2;
        int pad = 4;
        int minOff = onLeft + pad;
        int maxOff = onRight - total - pad;
        if (maxOff < minOff) maxOff = minOff;
        if (desired < minOff) desired = minOff;
        if (desired > maxOff) desired = maxOff;
        stringOffset = desired;
    }
    ...
    case lookGtk: {
        ...
        {
            int total2 = tlen + llen + rlen;
            // Center against the complete titlebar; clamp only when resources
            // or controls would make that position collide.
            int desired2 = int(width() - total2) / 2;
            int pad2 = 2;
            int min2 = lLeft + pad2;
            int max2 = lRight - total2 - pad2;
            if (max2 < min2) max2 = min2;
            if (desired2 < min2) desired2 = min2;
            if (desired2 > max2) desired2 = max2;
            stringOffset = desired2;
        }
...
void YFrameTitleBar::renderShape(Graphics& g) {
    ...
    if (LOOK(lookPixmap | lookMetal | lookGtk | lookFlat))
    {
        ...
        int stringOffset3;
        {
            int total3 = tlen + llen + rlen;
            int desired3 = x() + int(width() - total3) / 2;
            int pad3 = 2;
            int min3 = lLeft + pad3;
            int max3 = lRight - total3 - pad3;
            if (max3 < min3) max3 = min3;
            if (desired3 < min3) desired3 = min3;
            if (desired3 > max3) desired3 = max3;
            stringOffset3 = desired3;
        }
        lLeft = stringOffset3;
        lRight = stringOffset3 + tlen + llen + rlen;
