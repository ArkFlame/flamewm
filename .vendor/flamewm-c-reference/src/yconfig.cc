...
bool YConfig::consumeRuntimeEffective() {
#ifdef FLAMEWM_PRODUCT_BUILD
    flamewm::Runtime* rt = flamewm::Runtime::instance();
    if (!rt || !rt->initialized() || rt->stopping())
        return false;
    // Only Flame-owned conceptual keys should flow via Runtime; fall back to file
    // for legacy IceWM prefs. Currently we expose opacity/taskbar/accent/icon-theme
    // via Runtime effective snapshot by applying them to live globals when present.
    // Return false so file path remains for generic prefs; hook is opt-in per caller.
    (void)rt;
#endif
    return false;
}
