...
// FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-DESKTOP-01)
#ifndef FLAMEDESKTOP_EXE
#define FLAMEDESKTOP_EXE "flamewm-desktop"
#endif
// FLAMEWM-UPSTREAM-HOOK-END(FW-DESKTOP-01)
...
class SessionManager: private YApplication, private YTimerListener {
private:
    char* trim(char *line) {
    ...
    bool icewmExplicit;
    bool shuttingDown;
    mstring defaultWmStorage;
    int desktop_pid;
    int desktopRestarts;
    bool desktopGaveUp;
    YTimer desktopTimer;
    static const int kDesktopMaxRestarts = 5;
    ...
    void deriveDefaultWM(const char* inv) {
        const char* base = baseName(inv);
        flameMode = (strcmp(base, "flamewm-session") == 0);
        if (flameMode) {
            const char* ice = ICEWMEXE;
            const char* hit = strstr(ice, "icewm");
            if (hit) {
                size_t pre = size_t(hit - ice);
                mstring tmp(ice, pre);
                tmp += "flamewm";
                tmp += (hit + 5);
                defaultWmStorage = tmp;
            } else {
                defaultWmStorage = "flamewm";
            }
            icewmExe = defaultWmStorage.c_str();
        } else {
            defaultWmStorage = ICEWMEXE;
            icewmExe = defaultWmStorage.c_str();
        }
        icewmExplicit = false;
    }
        ...
        for (char **arg = 1 + *argv; arg < *argv + *argc; ++arg) {
            if (**arg == '-') {
                ...
                if (GetArgument(value, "d", "display", arg, *argv+*argc)) {
                    ...
                    icewmExe = value;
                    icewmExplicit = true;
                    defaultWmStorage = icewmExe ? icewmExe : "";
                    if (icewmExplicit)
                        icewmExe = defaultWmStorage.c_str();
                }
...
public:
    ...
    SessionManager(int *argc, char ***argv): YApplication(argc, argv) {
        ...
        desktopGaveUp = false;
        shuttingDown = false;
        desktopTimer.setTimerListener(this);
        crashtime = zerotime();
    ...
    // YTimerListener for flamewm-desktop bounded restart
    virtual bool handleTimer(YTimer* t) {
        if (t == &desktopTimer) {
            desktopTimer.stopTimer();
            if (!shuttingDown && flameMode && !desktopGaveUp && desktop_pid == -1) {
                runDesktop();
            }
            return true;
        }
    ...
    void scheduleDesktopRestart(long ms) {
        if (shuttingDown || !flameMode || desktopGaveUp)
            return;
        if (desktopRestarts >= kDesktopMaxRestarts) {
            if (!desktopGaveUp) {
                desktopGaveUp = true;
                warn(_("flamewm-desktop: giving up after %d restarts; WM remains usable"),
                     kDesktopMaxRestarts);
            }
            return;
        }
        desktopTimer.setInterval(ms);
    ...
    void runDesktop(bool quit = false) {
        if (!flameMode)
            return;
        if (quit) {
            shuttingDown = true;
            ...
            if (desktop_pid != -1) {
                ...
                // reap with timeout: waitpid nonblock; caller may wait, but SIGCHLD will reap
                int status = 0;
                pid_t r = waitpid(desktop_pid, &status, WNOHANG);
                if (r == 0) {
                    // give it a moment then SIGKILL if still alive during shutdown - caller may sync wait
                    fsleep(0.5);
                    if (kill(desktop_pid, 0) == 0) {
                        kill(desktop_pid, SIGKILL);
                        waitpid(desktop_pid, &status, 0);
                    }
                }
                desktop_pid = -1;
            }
        ...
        if (shuttingDown || desktopGaveUp)
            return;
        if (desktop_pid != -1)
            return;
        const char *args[4] = { FLAMEDESKTOP_EXE, nullptr, nullptr, nullptr };
        appendOptions(args, 1, ACOUNT(args));
        desktop_pid = runProgram(args[0], args);
        if (desktop_pid <= 0) {
            desktop_pid = -1;
            warn(_("failed to start %s"), FLAMEDESKTOP_EXE);
            // treat as crash for backoff counting
            checkDesktopExitStatus(0);
        } else {
            MSG(("started %s pid=%d", FLAMEDESKTOP_EXE, desktop_pid));
        }
    }

    void checkDesktopExitStatus(int status) {
        if (shuttingDown)
            return;
        if (!flameMode)
            return;
        ...
        if (WIFEXITED(status)) {
            ...
            if (code == 0) {
                // clean exit outside shutdown: still restart unless shutting down; but avoid storm
                // count it as restart; if intentional user quit, bounded restart will eventually stop
                MSG(("%s exited cleanly status 0", FLAMEDESKTOP_EXE));
            } else {
                tlog(_("%s exited with status %d."), FLAMEDESKTOP_EXE, code);
            }
        } else if (WIFSIGNALED(status)) {
            tlog(_("%s was killed by signal %d."), FLAMEDESKTOP_EXE, WTERMSIG(status));
        } else {
            MSG(("%s exited status=%d", FLAMEDESKTOP_EXE, status));
        }
        if (doRestart) {
            if (desktopRestarts >= kDesktopMaxRestarts) {
                desktopGaveUp = true;
            ...
            long backoffMs = 500L << desktopRestarts; // 500,1000,2000,4000,8000
            if (backoffMs > 8000L) backoffMs = 8000L;
            desktopRestarts++;
            MSG(("scheduling %s restart %d/%d in %ldms",
                 FLAMEDESKTOP_EXE, desktopRestarts, kDesktopMaxRestarts, backoffMs));
            scheduleDesktopRestart(backoffMs);
        }
    }
...
private:
    ...
    void notified() {
        if (++startup_phase == 1)
            ...
            runScript("startup");
            if (flameMode && !shuttingDown && !desktopGaveUp && desktop_pid == -1) {
                // start desktop after session/X ready (post-startup); bounded supervision thereafter
                runDesktop();
            }
        } else if (startup_phase == 3) {
            // reserved: desktop notify not required; desktop supervision is independent
        }
    }

public:
    void startDesktopIfNeeded() { if (flameMode) runDesktop(); }
    void stopDesktop() { runDesktop(true); }
