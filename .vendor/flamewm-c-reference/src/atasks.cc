...
TaskPane::TaskPane(IAppletContainer* taskBar, YWindow* parent):
    ...
    fDragX(0),
    fDragY(0),
    fDragThresholdMet(false),
    fNeedRelayout(true),
    fForceImmediate(false),
    fRemoveAtWorkspace(-1),
    // Flame task strip keeps every frame independently addressable.
    fTaskGrouping(0)
{
...
// FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-TASK-01)
int TaskPane::buttonCount() const {
    return fTasks.getCount();
}

TaskButton* TaskPane::buttonAt(int index) const {
    return index >= 0 && index < fTasks.getCount() ? fTasks[index] : nullptr;
}

int TaskPane::indexOfButton(TaskButton* button) const {
    for (int index = 0; index < fTasks.getCount(); ++index) {
        if (fTasks[index] == button)
            return index;
    }
    return -1;
}

void TaskPane::insertButtonAt(TaskButton* button, int index) {
    if (button == nullptr || indexOfButton(button) >= 0)
        return;
    index = clamp(index, 0, fTasks.getCount());
    fTasks.insert(index, button);
    fRemoveAtWorkspace = -1;
}

void TaskPane::moveButtonTo(TaskButton* button, int index) {
    const int from = indexOfButton(button);
    if (from < 0 || fTasks.getCount() < 2)
        return;
    index = clamp(index, 0, fTasks.getCount() - 1);
    if (from == index)
        return;
    TaskButton* moved = fTasks[from];
    if (from < index) {
...
// FLAMEWM-UPSTREAM-HOOK-END(FW-TASK-01)
...
void TaskPane::regroup() {
    ...
    // Flame task strip never groups frames, including after preference reload.
    fTaskGrouping = 0;
    fNeedRelayout = true;
...
void TaskPane::relayoutNow(bool force) {
    if (fTaskGrouping != 0 && fTaskGrouping != taskBarTaskGrouping && !dragging())
        regroup();
...
void TaskPane::startDrag(TaskButton* drag, int /*byMouse*/, int sx, int sy) {
    if (fDragging == nullptr) {
                             ...
                             PointerMotionMask))
        {
            ...
            fDragY = sy;
            fDragThresholdMet = false;
            fDragOriginIndex = indexOfButton(drag);
        }
    }
}
...
void TaskPane::processDrag(int mx, int my) {
    ...
    const bool horizontal = taskButtonsHorizontal();
    const int pointer = horizontal ? mx : my;
    const int startPointer = horizontal ? fDragX : fDragY;
    const int delta = pointer - startPointer;
    const int threshold = 6;
    if (!fDragThresholdMet && abs(delta) < threshold)
        return;
    fDragThresholdMet = true;

    const int span = horizontal ? width() : height();
    const int buttonSpan = horizontal ? fDragging->width() : fDragging->height();
    const int w2 = span - buttonSpan - 2;
    const int ox = fDragX - fDragging->x();
    const int oy = fDragY - fDragging->y();
    const int cx = horizontal ? clamp(mx, ox, ox + w2) : fDragging->x() + fDragging->width() / 2;
    const int cy = horizontal ? fDragging->y() + fDragging->height() / 2 : clamp(my, oy, oy + w2);
    const int dx = horizontal ? cx - fDragX : 0;
    const int dy = horizontal ? 0 : cy - fDragY;
    if (dx == 0 && dy == 0)
        return;

    fDragging->setPosition(fDragging->x() + dx, fDragging->y() + dy);

    const int drag = indexOfButton(fDragging);
    const int cursor = horizontal ? cx : cy;
    int dropPhysical = -1;
    bool haveDrop = false;
    for (IterTask task = fTasks.iterator(); ++task; ) {
        if (!task->getShown() || task == fDragging)
            continue;
        const int mid = horizontal
            ? task->x() + int(task->width() / 2)
            : task->y() + int(task->height() / 2);
        if (cursor < mid) {
            dropPhysical = task.where();
            haveDrop = true;
            break;
        }
    }
    if (!haveDrop) {
        // insertion after last shown (or append if none shown besides drag)
        // check if there is at least one other shown task
        bool hasOtherShown = false;
        for (IterTask task = fTasks.iterator(); ++task; ) {
            if (task->getShown() && task != fDragging) { hasOtherShown = true; break; }
        }
        if (!hasOtherShown)
            ; // no drop target
        else
            dropPhysical = fTasks.getCount(); // append after last
        haveDrop = hasOtherShown;
    }
    if (haveDrop && drag >= 0 && dropPhysical >= 0) {
        int target = dropPhysical;
        if (drag >= 0 && drag < dropPhysical)
            target = dropPhysical - 1;
        target = clamp(target, 0, fTasks.getCount() - 1);
        if (target != drag)
            moveButtonTo(fDragging, target);
    }
    fDragX += dx;
    fDragY += dy;
    relayout(true);
...
void TaskPane::cancelDrag() {
    if (fDragging) {
        TaskButton* drag = fDragging;
        const int origin = fDragOriginIndex;
        const bool mutated = fDragThresholdMet;
        // release grab exactly once
        xapp->releaseEvents();
        drag->deselect();
        fDragging = nullptr;
        fDragThresholdMet = false;
        fDragOriginIndex = -1;
        if (mutated && origin >= 0 && origin < fTasks.getCount()) {
            const int cur = indexOfButton(drag);
            if (cur >= 0 && cur != origin) {
                // restore original ordered identity
                moveButtonTo(drag, origin);
            }
        } else if (mutated && origin >= 0) {
            const int cur = indexOfButton(drag);
            if (cur >= 0) {
                const int clamped = clamp(origin, 0, fTasks.getCount() - 1);
                if (cur != clamped)
                    moveButtonTo(drag, clamped);
            }
        }
        relayout(true);
    } else {
        // ensure origin cleared even if dragging already null (re-entrancy)
        fDragThresholdMet = false;
        fDragOriginIndex = -1;
    }
}
