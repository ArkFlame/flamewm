...
#ifdef FLAMEWM_PRODUCT_BUILD
namespace flamewm { struct SettingsSnapshot; }
#endif
...
public:
...
#ifdef FLAMEWM_PRODUCT_BUILD
    // FW-HOTKEY-01 staged-grab transaction — narrow adapter for registry grab contract.
    // Keeps YWindowManager::grabKeys as X ownership point.
    bool hotkeyGrabAdapter(const std::string& keysym);
    bool applyFlameHotkeyTransaction(const std::map<std::string, std::string>& hotkeys, std::string* error);
    bool stagedHotkeyGrabTransaction(const std::map<int, std::string>& desiredByActionInt, std::string* error);
    static bool hotkeyEffectStager(const flamewm::SettingsSnapshot& cand, std::string* error, void* ctx);
    void ensureHotkeyStager();
#endif
    ...
    // RandR registration stays in YWindowManager ctor (XRRSelectInput).
    // Apply/Revert delegates to Runtime-owned DisplayManager (WM owns tx, Settings is UI only).
    bool flameRequestDisplayMode(const std::string& outputKey, uint32_t modeId, uint64_t timeoutMs, std::string* error);
    bool flameKeepDisplayTx(std::string* error);
    bool flameRevertDisplayTx(std::string* error);
#endif
    ...
    // Insert/remove delegate to WorkspaceAdapter via Bridge when attached (product).
    // Vanilla fallback preserves upstream behavior. Native atomics live behind this seam.
    bool insertWorkspaceAt(int index);
    bool removeWorkspaceAt(int index);
#ifdef FLAMEWM_PRODUCT_BUILD
    bool insertWorkspaceAtNative(int index);
    bool removeWorkspaceAtNative(int index);
#endif
