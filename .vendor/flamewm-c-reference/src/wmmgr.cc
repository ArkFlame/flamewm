...
#include "ystring.h"
#include "keysyms.h"
// FLAMEWM-BRIDGE-HOOK-BEGIN(FW-WS-01)
...
#endif
// FLAMEWM-BRIDGE-HOOK-END(FW-WS-01)
// FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-HOTKEY-01)
#ifdef FLAMEWM_PRODUCT_BUILD
#include "flamewm/core/shortcutregistry.h"
#include "flamewm/core/config.h"
#include "flamewm/core/runtime.h"
#include "wmkey.h"
#include "bindkey.h"
#include <X11/Xlib.h>
// FLAMEWM-UPSTREAM-HOOK-END(FW-HOTKEY-01)
#endif
#include "intl.h"
#include "ywordexp.h"
#include <string>
...
// FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-HOTKEY-01)
#ifdef FLAMEWM_PRODUCT_BUILD
// Narrow staged-grab adapter implementing registry's existing grab contract.
// Keeps YWindowManager::grabKeys as X ownership point.
// Transaction: desired map -> normalize+validate -> duplicate detection
// -> stage candidate WMKey values -> XSync -> scoped X error capture
// -> attempt all candidate grabs -> XSync
// Failure: ungrab staged, restore previous, registry unchanged, revision unchanged
// Success: release obsolete old grabs, publish registry state, then allow revision commit
// No global persistent X error-handler corruption.
namespace {
static bool s_hotkeyGrabFailed = false;
static XErrorHandler s_prevHotkeyHandler = 0;
static int hotkeyXErrorHandler(Display* dpy, XErrorEvent* ev) {
    if (ev->error_code == BadAccess || ev->error_code == BadValue || ev->error_code == BadWindow) {
        s_hotkeyGrabFailed = true;
        return 0;
    }
    if (s_prevHotkeyHandler)
        return s_prevHotkeyHandler(dpy, ev);
    return 0;
}
static flamewm::ShortcutRegistry* flameRegistry() {
    static flamewm::ShortcutRegistry r;
    return &r;
}
static bool actionFromName(const std::string& name, flamewm::ShortcutAction* out) {
    if (name == "ToggleStartMenu") { *out = flamewm::ActionToggleStartMenu; return true; }
    if (name == "WorkspaceLeft") { *out = flamewm::ActionWorkspaceLeft; return true; }
    if (name == "WorkspaceRight") { *out = flamewm::ActionWorkspaceRight; return true; }
    if (name == "WorkspaceUp") { *out = flamewm::ActionWorkspaceUp; return true; }
    if (name == "WorkspaceDown") { *out = flamewm::ActionWorkspaceDown; return true; }
    if (name == "WindowClose") { *out = flamewm::ActionWindowClose; return true; }
    if (name == "WindowMinimize") { *out = flamewm::ActionWindowMinimize; return true; }
    if (name == "WindowMaximize") { *out = flamewm::ActionWindowMaximize; return true; }
    return false;
}
static bool hotkeyGrabAdapterFn(const std::string& keysym, void* ctx) {
    YWindowManager* self = static_cast<YWindowManager*>(ctx);
    return self->hotkeyGrabAdapter(keysym);
}
} // anon

bool YWindowManager::hotkeyGrabAdapter(const std::string& keysym) {
    if (keysym.empty())
        return true;
    std::string n = flamewm::ShortcutRegistry::normalize(keysym);
    if (n.empty())
        return true;
    if (flamewm::ShortcutRegistry::isEscape(n) || flamewm::ShortcutRegistry::isEscapeNormalized(n))
        return false;
    WMKey wk;
    if (!wk.set(n.c_str()))
        return false;
...
bool YWindowManager::stagedHotkeyGrabTransaction(const std::map<int, std::string>& desiredByInt, std::string* error) {
    flamewm::ShortcutRegistry* reg = flameRegistry();
    std::map<flamewm::ShortcutAction, std::string> desired;
    for (std::map<int, std::string>::const_iterator it = desiredByInt.begin(); it != desiredByInt.end(); ++it) {
        int ai = it->first;
        if (ai < 0 || ai >= (int)flamewm::ActionCount) {
            if (error) *error = "unknown action";
            return false;
        }
        std::string v = it->second;
        std::string n = flamewm::ShortcutRegistry::normalize(v);
        if (flamewm::ShortcutRegistry::isEscape(v) || flamewm::ShortcutRegistry::isEscapeNormalized(n))
            v = "";
        desired[(flamewm::ShortcutAction)ai] = v;
    }
    for (std::map<flamewm::ShortcutAction, std::string>::const_iterator it = desired.begin(); it != desired.end(); ++it) {
        std::string n = flamewm::ShortcutRegistry::normalize(it->second);
        if (n.empty()) continue;
        unsigned k = 0; unsigned short m = 0;
        if (!YConfig::parseKey(n.c_str(), &k, &m)) {
            if (error) *error = "invalid keysym: " + n;
            return false;
        }
    }
    Display* dpy = (xapp ? xapp->display() : 0);
    if (!dpy) {
        return reg->applyWithGrabStage(desired, 0, 0, error);
    }
    std::map<flamewm::ShortcutAction, flamewm::KeyBinding> oldMap;
    for (int a = 0; a < (int)flamewm::ActionCount; ++a)
        oldMap[(flamewm::ShortcutAction)a] = reg->bindingFor((flamewm::ShortcutAction)a);
    s_hotkeyGrabFailed = false;
    XSync(dpy, False);
    XErrorHandler prev = XSetErrorHandler(hotkeyXErrorHandler);
    s_prevHotkeyHandler = prev;
    bool regOk = reg->applyWithGrabStage(desired, hotkeyGrabAdapterFn, this, error);
    XSync(dpy, False);
    XSetErrorHandler(prev);
    s_prevHotkeyHandler = 0;
    XSync(dpy, False);
    if (!regOk) {
        for (std::map<flamewm::ShortcutAction, std::string>::const_iterator it = desired.begin(); it != desired.end(); ++it) {
            if (!it->second.empty()) {
                WMKey wk; wk.set(it->second.c_str()); wk.ungrab(handle());
            }
        }
        return false;
    }
    if (s_hotkeyGrabFailed) {
        for (int a = 0; a < (int)flamewm::ActionCount; ++a) {
            flamewm::ShortcutAction act = (flamewm::ShortcutAction)a;
            flamewm::KeyBinding ob = oldMap[act];
            if (ob.assigned && !ob.keysym.empty())
                reg->setBindingValidated(act, ob.keysym, 0);
            else
                reg->clearBinding(act);
        }
        for (std::map<flamewm::ShortcutAction, std::string>::const_iterator it = desired.begin(); it != desired.end(); ++it) {
            if (!it->second.empty()) {
                WMKey wk; wk.set(it->second.c_str()); wk.ungrab(handle());
        ...
        if (error && error->empty()) *error = "X grab failed";
        return false;
    }
    for (int a = 0; a < (int)flamewm::ActionCount; ++a) {
        flamewm::ShortcutAction act = (flamewm::ShortcutAction)a;
        std::string oldKs = oldMap[act].assigned ? oldMap[act].keysym : "";
        flamewm::KeyBinding nb = reg->bindingFor(act);
        std::string newKs = nb.assigned ? nb.keysym : "";
        if (oldKs != newKs && !oldKs.empty()) {
            WMKey wk; wk.set(oldKs.c_str()); wk.ungrab(handle());
        }
    }
    struct MapEnt { flamewm::ShortcutAction act; WMKey* key; };
    MapEnt maps[] = {
        { flamewm::ActionToggleStartMenu, &gKeyFlameToggleStartMenu },
        { flamewm::ActionWorkspaceLeft, &gKeyFlameWorkspaceLeft },
        { flamewm::ActionWorkspaceRight, &gKeyFlameWorkspaceRight },
        { flamewm::ActionWorkspaceUp, &gKeyFlameWorkspaceUp },
        { flamewm::ActionWorkspaceDown, &gKeyFlameWorkspaceDown },
        { flamewm::ActionWindowClose, &gKeyFlameWindowClose },
        { flamewm::ActionWindowMinimize, &gKeyFlameWindowMinimize },
        { flamewm::ActionWindowMaximize, &gKeyFlameWindowMaximize },
    };
    for (size_t i = 0; i < sizeof(maps)/sizeof(maps[0]); ++i) {
        flamewm::KeyBinding b = reg->bindingFor(maps[i].act);
        if (b.assigned && !b.keysym.empty()) maps[i].key->set(b.keysym.c_str());
        else maps[i].key->set("");
    }
    return true;
}

bool YWindowManager::applyFlameHotkeyTransaction(const std::map<std::string, std::string>& hotkeys, std::string* error) {
    std::map<int, std::string> desiredByInt;
    for (std::map<std::string, std::string>::const_iterator it = hotkeys.begin(); it != hotkeys.end(); ++it) {
        flamewm::ShortcutAction act;
        if (!actionFromName(it->first, &act)) {
            if (error) *error = "unknown hotkey action: " + it->first;
            return false;
        }
        std::string v = it->second;
        std::string n = flamewm::ShortcutRegistry::normalize(v);
        if (flamewm::ShortcutRegistry::isEscape(v) || flamewm::ShortcutRegistry::isEscapeNormalized(n))
            v = "";
        desiredByInt[(int)act] = v;
    }
    return stagedHotkeyGrabTransaction(desiredByInt, error);
}

bool YWindowManager::hotkeyEffectStager(const flamewm::SettingsSnapshot& cand, std::string* error, void* ctx) {
    YWindowManager* self = static_cast<YWindowManager*>(ctx);
    if (!self) return false;
    if (cand.hotkeys.empty()) return true;
    return self->applyFlameHotkeyTransaction(cand.hotkeys, error);
}

void YWindowManager::ensureHotkeyStager() {
    flamewm::Runtime* rt = flamewm::Runtime::instance();
    if (!rt) return;
    rt->setEffectStager(&YWindowManager::hotkeyEffectStager, this);
    const flamewm::SettingsSnapshot& eff = rt->effectiveSettings();
    if (!eff.hotkeys.empty()) {
        std::string err;
        applyFlameHotkeyTransaction(eff.hotkeys, &err);
    } else {
        flamewm::ShortcutRegistry* reg = flameRegistry();
        struct MapEnt2 { flamewm::ShortcutAction act; WMKey* key; };
        MapEnt2 maps[] = {
            { flamewm::ActionToggleStartMenu, &gKeyFlameToggleStartMenu },
            { flamewm::ActionWorkspaceLeft, &gKeyFlameWorkspaceLeft },
            { flamewm::ActionWorkspaceRight, &gKeyFlameWorkspaceRight },
...
bool YWindowManager::handleWMKey(const XKeyEvent& key, bool repeating) {
    ...
    // FLAMEWM-BRIDGE-HOOK-BEGIN(FW-START-01)
#ifdef FLAMEWM_PRODUCT_BUILD
    else if (gKeyFlameToggleStartMenu == key &&
             flamewm::engine::icewm::Hooks::rootKey(key.keycode, key.state)) {
        XAllowEvents(xapp->display(), AsyncKeyboard, key.time);
        return true;
    }
#endif
    // FLAMEWM-BRIDGE-HOOK-END(FW-START-01)
    // FLAMEWM-BRIDGE-HOOK-BEGIN(FW-WS-NAV)
#ifdef FLAMEWM_PRODUCT_BUILD
    else if (gKeyFlameWorkspaceLeft == key || gKeyFlameWorkspaceRight == key ||
             gKeyFlameWorkspaceUp == key || gKeyFlameWorkspaceDown == key) {
        XAllowEvents(xapp->display(), AsyncKeyboard, key.time);
        auto* bhost = flamewm::engine::icewm::Bridge::instance().host();
        if (flamewm::engine::icewm::Bridge::instance().isAttached() && bhost && bhost->workspaces()) {
            auto* ws = bhost->workspaces();
            if (gKeyFlameWorkspaceLeft == key) ws->navigateLeft();
            else if (gKeyFlameWorkspaceRight == key) ws->navigateRight();
            else if (gKeyFlameWorkspaceUp == key) ws->navigateUp();
            else ws->navigateDown();
            return true;
        }
        int nw = activeWorkspace();
        if (gKeyFlameWorkspaceLeft == key) nw = flamewm::workspace::TwoRowTopology::moveLeft(activeWorkspace(), workspaceCount);
        else if (gKeyFlameWorkspaceRight == key) nw = flamewm::workspace::TwoRowTopology::moveRight(activeWorkspace(), workspaceCount);
        else if (gKeyFlameWorkspaceUp == key) nw = flamewm::workspace::TwoRowTopology::moveUp(activeWorkspace(), workspaceCount);
        else nw = flamewm::workspace::TwoRowTopology::moveDown(activeWorkspace(), workspaceCount);
        switchToWorkspace(nw, false);
        return true;
    }
#endif
    // FLAMEWM-BRIDGE-HOOK-END(FW-WS-NAV)
...
void YWindowManager::handleDestroyWindow(const XDestroyWindowEvent &destroyWindow) {
    YDesktop::handleDestroyWindow(destroyWindow);
}
...
bool YWindowManager::updateWorkAreaInner() {
    ...
    if (oldWorkArea == nullptr)
        ...
        int areaCount = ::workspaceCount * screenCount;
        ...
        if (oldWorkArea[0] == nullptr) {
            ...
            fWorkAreaScreenCount = screenCount;
...
#ifdef FLAMEWM_PRODUCT_BUILD
bool YWindowManager::insertWorkspaceAtNative(int index) {
    int count = workspaceCount;
    if (count >= NewMaxWorkspaces) return false;
    if (index < 0 || index > count) return false;
    std::vector<std::string> names;
    names.reserve(count);
    for (int i = 0; i < count; ++i) names.push_back(workspaceNames[i] ? workspaceNames[i] : "");
    std::vector<int> frameWs;
    for (YFrameIter it = focusedIterator(); ++it; ) frameWs.push_back(it->getWorkspace());
    auto r = flamewm::workspace::WorkspaceTransaction::insertWorkspace(count, index, names, frameWs, fActiveWorkspace, fLastWorkspace, 1);
    if (!r.valid) return false;
    workspaces.reset();
    for (size_t i = 0; i < r.newNames.size(); ++i) workspaces.add(r.newNames[i].c_str());
    {
        YWorkAreaLock wal;
        lockFocus();
        int fi = 0;
        for (YFrameIter it = focusedIterator(); ++it; ) {
            int newWs = r.frameNewWs[fi++];
            if (newWs >= 0) it->setWorkspace(newWs);
        }
        fActiveWorkspace = r.newActive;
        fLastWorkspace = r.newLast;
        setDesktopNames((int)r.newNames.size());
        setDesktopCount();
        setDesktopViewport();
        setProperty(_XA_NET_CURRENT_DESKTOP, XA_CARDINAL, fActiveWorkspace);
        updateWorkArea();
        if (taskBar) { taskBar->workspacesUpdateButtons(); taskBar->workspacesRelabelButtons(); }
        if (windowList) windowList->updateWorkspaces();
        updateMoveMenu();
        unlockFocus();
    }
    return true;
}
bool YWindowManager::removeWorkspaceAtNative(int index) {
    int count = workspaceCount;
    ...
    std::vector<int> frameWs;
    for (YFrameIter it = focusedIterator(); ++it; ) frameWs.push_back(it->getWorkspace());
    auto r = flamewm::workspace::WorkspaceTransaction::removeWorkspace(count, index, names, frameWs, fActiveWorkspace, fLastWorkspace, 1);
    if (!r.valid) return false;
    std::string removedName = (index < (int)names.size() ? names[index] : "");
    bool removingActive = (fActiveWorkspace == index);
    if (removingActive) setFocus(nullptr);
    {
        YWorkAreaLock wal;
        lockFocus();
        int fi = 0;
        ...
        workspaces.reset();
        for (size_t i = 0; i < r.newNames.size(); ++i) workspaces.add(r.newNames[i].c_str());
        if (!removedName.empty()) workspaces.spare(index, removedName.c_str());
        fActiveWorkspace = r.newActive;
        fLastWorkspace = r.newLast;
        setDesktopNames((int)r.newNames.size());
        setDesktopCount();
    ...
    if (removingActive) { activateWorkspace(fActiveWorkspace); focusLastWindow(); }
    return true;
}
#endif

bool YWindowManager::insertWorkspaceAt(int index) {
#ifdef FLAMEWM_PRODUCT_BUILD
    auto* host = flamewm::engine::icewm::Bridge::instance().host();
    if (flamewm::engine::icewm::Bridge::instance().isAttached() && host && host->workspaces()) {
        uint64_t rev = host->workspaces()->revision();
        flamewm::api::WorkspaceTransform t = flamewm::api::WorkspaceTransform::InsertAfter(index - 1, rev);
        return host->workspaces()->applyTransform(t).ok();
    }
    return insertWorkspaceAtNative(index);
#else
    int count = workspaceCount;
    if (count >= NewMaxWorkspaces) return false;
    if (index < 0 || index > count) return false;
    std::vector<std::string> names;
    for (int i = 0; i < count; ++i) names.push_back(workspaceNames[i] ? workspaceNames[i] : "");
    names.insert(names.begin() + index, "Workspace");
    workspaces.reset();
    for (size_t i = 0; i < names.size(); ++i) workspaces.add(names[i].c_str());
    for (YFrameIter it = focusedIterator(); ++it; )
        if (it->getWorkspace() >= index) it->setWorkspace(it->getWorkspace() + 1);
    if (fActiveWorkspace >= index) ++fActiveWorkspace;
    if (fLastWorkspace >= index) ++fLastWorkspace;
    setDesktopNames(count + 1); setDesktopCount(); setDesktopViewport(); updateWorkArea();
    if (taskBar) { taskBar->workspacesUpdateButtons(); taskBar->workspacesRelabelButtons(); }
    if (windowList) windowList->updateWorkspaces();
    updateMoveMenu();
    return true;
...
bool YWindowManager::removeWorkspaceAt(int index) {
#ifdef FLAMEWM_PRODUCT_BUILD
    auto* host = flamewm::engine::icewm::Bridge::instance().host();
    if (flamewm::engine::icewm::Bridge::instance().isAttached() && host && host->workspaces()) {
        uint64_t rev = host->workspaces()->revision();
        flamewm::api::WorkspaceTransform t = flamewm::api::WorkspaceTransform::Remove(index, rev);
        return host->workspaces()->applyTransform(t).ok();
    }
    return removeWorkspaceAtNative(index);
#else
    int count = workspaceCount;
    if (count <= 1) return false;
    if (index < 0 || index >= count) return false;
    std::vector<std::string> names;
    for (int i = 0; i < count; ++i) names.push_back(workspaceNames[i] ? workspaceNames[i] : "");
    std::string removedName = names[index];
    for (YFrameIter it = focusedIterator(); ++it; ) {
        int ws = it->getWorkspace();
        if (ws == index) it->setWorkspace(index == count - 1 ? count - 2 : index);
        else if (ws > index) it->setWorkspace(ws - 1);
    }
    if (fActiveWorkspace == index) fActiveWorkspace = index == count - 1 ? count - 2 : index;
    else if (fActiveWorkspace > index) --fActiveWorkspace;
    if (fLastWorkspace == index) fLastWorkspace = fActiveWorkspace;
    else if (fLastWorkspace > index) --fLastWorkspace;
    workspaces.reset();
    for (int i = 0; i < count; ++i) if (i != index) workspaces.add(names[i].c_str());
    if (!removedName.empty()) workspaces.spare(index, removedName.c_str());
    setDesktopNames(count - 1); setDesktopCount(); setDesktopViewport(); updateWorkArea();
    if (taskBar) { taskBar->workspacesUpdateButtons(); taskBar->workspacesRelabelButtons(); }
    if (windowList) windowList->updateWorkspaces();
...
#ifdef FLAMEWM_PRODUCT_BUILD
    // FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-DISPLAY-01)
    // Keep existing IceWM work-area/desks handling; delegate display tx hotplug to Runtime.
    // Generates fresh RandR topology with new generation; Runtime validates + reverts if stale.
    if (flamewm::Runtime::instance())
        flamewm::Runtime::instance()->onDisplayHotplug(true);
#endif
    // FLAMEWM-UPSTREAM-HOOK-END(FW-DISPLAY-01)
...
// FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-DISPLAY-01)
#ifdef FLAMEWM_PRODUCT_BUILD
bool YWindowManager::flameRequestDisplayMode(const std::string& outputKey, uint32_t modeId, uint64_t timeoutMs, std::string* error) {
    flamewm::Runtime* rt = flamewm::Runtime::instance();
    if (!rt) { if (error) *error = "no runtime"; return false; }
    const flamewm::OutputInfo* out = rt->displayManager()->snapshot().findByDurable(outputKey);
    if (!out) { if (error) *error = "output not found"; return false; }
    flamewm::DisplayMode mode;
    bool found = false;
    for (size_t i = 0; i < out->availableModes.size(); ++i) if (out->availableModes[i].id == modeId) { mode = out->availableModes[i]; found = true; break; }
    if (!found) { if (error) *error = "mode not available"; return false; }
    return rt->requestDisplayMode(outputKey, mode, timeoutMs, error);
}
bool YWindowManager::flameKeepDisplayTx(std::string* error) {
    flamewm::Runtime* rt = flamewm::Runtime::instance();
    if (!rt) { if (error) *error = "no runtime"; return false; }
    return rt->keepDisplayTx(error);
}
bool YWindowManager::flameRevertDisplayTx(std::string* error) {
    flamewm::Runtime* rt = flamewm::Runtime::instance();
    if (!rt) { if (error) *error = "no runtime"; return false; }
    return rt->revertDisplayTx(error);
}
// FLAMEWM-UPSTREAM-HOOK-END(FW-DISPLAY-01)
#endif
#endif
