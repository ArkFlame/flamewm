...
// FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-PANEL-01)
static YAction actionCreateVirtualDesktop;
static YAction actionTaskbarSettings;
// FLAMEWM-UPSTREAM-HOOK-END(FW-PANEL-01)
...
public:
    void updatePopup() {
        ...
        addItem(_("Create New Virtual Desktop"), -2, nullptr,
                actionCreateVirtualDesktop);
        addSeparator();
        addItem(_("Taskbar Settings..."), -2, nullptr, actionTaskbarSettings);
...
void TaskBar::updateLayout(unsigned &size_w, unsigned &size_h) {
    ...
    nw = LayoutInfo( fWorkspaces, taskBarWorkspacesLeft,
                     ...
                     taskBarShowWorkspaces && workspaceCount > 1,
                     Grow, 4, 4 );
    wlist.append(nw);
...
void TaskBar::actionPerformed(YAction action, unsigned int modifiers) {
    if (action == actionCreateVirtualDesktop) {
        manager->insertWorkspaceAt(workspaceCount);
    } else if (action == actionTaskbarSettings) {
        smActionListener->runCommand("flamewm-settings --page taskbar");
    } else {
...
void TaskBar::delistFrame(YFrameWindow* frame, TaskBarApp* task, TrayApp* tray) {
    ...
    if (frame) {
        char id[32];
        snprintf(id, sizeof id, "0x%lx", (unsigned long) frame->handle());
        fFlameTaskStrip.removeWindow(id);
    }
#endif
    if (taskPane() && task)
        taskPane()->remove(task);
#ifdef FLAMEWM_PRODUCT_BUILD
    fFlameTaskStrip.syncLaunchersToPane(taskPane());
#endif
...
TaskBarApp *TaskBar::addTasksApp(YFrameWindow* frame) {
    ...
    char id[32];
    snprintf(id, sizeof id, "0x%lx", (unsigned long) frame->handle());
    const ClassHint* hint = frame->classHint();
    const char* app = hint && nonempty(hint->res_class) ? hint->res_class
                     : hint && nonempty(hint->res_name) ? hint->res_name : id;
    fFlameTaskStrip.addWindow(app, id);
#endif
    TaskBarApp* result = taskPane()->addApp(frame);
#ifdef FLAMEWM_PRODUCT_BUILD
    fFlameTaskStrip.syncLaunchersToPane(taskPane());
#endif
    return result;
