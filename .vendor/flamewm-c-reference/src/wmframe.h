...
private:
    ...
    // Product: snap state lives in SnapService (Bridge). YFrameWindow holds
    // only a preview window token; no flamewm/snap headers here on product.
#ifdef FLAMEWM_PRODUCT_BUILD
    void* snapBridgeToken_;
    void snapBridgeMoveBegin();
    void snapBridgeMoveMotion(const XMotionEvent& m);
    void snapBridgeMoveEnd(bool cancel);
    void snapBridgeHidePreview();
#endif
