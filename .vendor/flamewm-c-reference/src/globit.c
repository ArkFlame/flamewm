        ...
        void(*callback)(const void *, const char * const *, unsigned cnt), const void* cback_user_parm)
{
	...
	if (is_absolute) {
		...
		case 1:
			*result = globit_escape(glob_block.gl_pathv[0]);
			break;
		default:
		    if(callback) callback(cback_user_parm,
#ifdef __cplusplus
		        reinterpret_cast<const char * const *>(glob_block.gl_pathv), i);
#else
		        (const char * const *)glob_block.gl_pathv, i);
#endif
 ...
 oom:
	...
	default:
        if (callback) callback(cback_user_parm,
#ifdef __cplusplus
            reinterpret_cast<const char * const *>(results), i);
#else
            (const char * const *)results, i);
#endif
