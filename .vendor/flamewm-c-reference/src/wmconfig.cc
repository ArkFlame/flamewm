...
void WMConfig::applyRuntimeEffective() {
#ifdef FLAMEWM_PRODUCT_BUILD
    flamewm::Runtime* rt = flamewm::Runtime::instance();
    if (!rt || !rt->initialized() || rt->stopping()) return;
    // Lazily register a persistent listener without static flamewm header dependency
    // at global scope — register once on first effective apply.
    class WMRuntimeSettingsListener : public flamewm::RuntimeSettingsListener {
    public:
        void onSettingsApplied(const flamewm::SettingsSnapshot&) override {
            WMConfig::applyRuntimeEffective();
        }
    };
    static WMRuntimeSettingsListener runtimeSettingsListener;
    rt->addSettingsListener(&runtimeSettingsListener);
    const flamewm::SettingsSnapshot& s = rt->effectiveSettings();
    // Only the icon theme bridges into existing IceWM/freedesktop ownership.
    // Taskbar, accent, and display values remain Flame-owned downstream state.
    if (!s.iconTheme.empty() && (!iconThemes || strcmp(iconThemes, s.iconTheme.c_str()) != 0))
        iconThemes = newstr(s.iconTheme.c_str());
#else
    (void)0;
#endif
}
