...
#ifdef FLAMEWM_PRODUCT_BUILD
    // Register product fonts before IceWM resolves configured UI fonts/themes.
...
#ifdef FLAMEWM_PRODUCT_BUILD
    flamewm::engine::icewm::Bootstrap::attachIfProduct(manager, this);
#endif
...
#ifdef FLAMEWM_PRODUCT_BUILD
    // FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-CORE-01)
    runtime = new flamewm::Runtime();
    // FW-CONFIG-01: Runtime effective snapshot authority. Resolve config path to XDG flame.conf;
    // Settings does not directly become effective — Runtime publishes once after validate->stage->persist.
    {
        std::string flamePath = flamewm::Runtime::defaultConfigPath();
        runtime->setConfigPath(flamePath);
    }
    PRECONDITION(runtime->initialize(manager));
    // WM consumes Runtime effective snapshot, not direct file reads where owned.
    WMConfig::applyRuntimeEffective();
    // FLAMEWM-UPSTREAM-HOOK-BEGIN(FW-HOTKEY-01)
    manager->ensureHotkeyStager();
    // FLAMEWM-UPSTREAM-HOOK-END(FW-HOTKEY-01)
    // FLAMEWM-UPSTREAM-HOOK-END(FW-CORE-01)
#endif
...
YWMApp::~YWMApp() {
    // FLAMEWM-BRIDGE-HOOK-BEGIN
#ifdef FLAMEWM_PRODUCT_BUILD
    flamewm::engine::icewm::Bootstrap::detach();
#endif
    // FLAMEWM-BRIDGE-HOOK-END
#ifdef FLAMEWM_PRODUCT_BUILD
    if (runtime)
