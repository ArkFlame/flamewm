...
YFrameWindow::~YFrameWindow() {
    ...
    // FLAMEWM-BRIDGE-HOOK-BEGIN(FW-SNAP-02)
#ifdef FLAMEWM_PRODUCT_BUILD
    if (snapBridgeToken_)
        flamewm::engine::icewm::SnapBridge::instance().cancel();
#endif
    // FLAMEWM-BRIDGE-HOOK-END(FW-SNAP-02)
...
void YFrameWindow::wmTile(YAction action) {
    ...
    if (notState(WinStateUnmapped | WinStateFullscreen)) {
        ...
        case actionTileBottomRight:
            ...
            y = my + (My - my) / 2;
            break;
// FLAMEWM-UPSTREAM-HOOK-BEGIN(G-01)
        case actionTileCenter:
            if (size)
                w = (Mx - mx) / 2, h = (My - my) / 2;
            x = mx + (Mx - mx - w) / 2;
            y = my + (My - my - h) / 2;
            break;
// FLAMEWM-UPSTREAM-HOOK-END(G-01)
        default: return;
        }
        if (size) {
...
// FLAMEWM-BRIDGE-HOOK-BEGIN(FW-SNAP-02)
// Product builds route snap through SnapService (Bridge); vanilla fallback retains old per-frame state.
#ifdef FLAMEWM_PRODUCT_BUILD
void YFrameWindow::snapBridgeMoveBegin() {
    if (!flamewm::engine::icewm::Bridge::instance().isAttached()) return;
    auto* h = flamewm::engine::icewm::Bridge::instance().host();
    if (!h || !h->snap()) return;
    Window w = client() ? client()->handle() : handle();
    if (w == None) w = handle();
    flamewm::api::WindowRef ref = flamewm::engine::icewm::WindowAdapter::currentRef(
        static_cast<uint64_t>(static_cast<unsigned long>(w)));
    if (!ref.valid()) return;
    flamewm::api::Rect geo(geometry().x(), geometry().y(), (int)geometry().width(), (int)geometry().height());
    if (flamewm::engine::icewm::SnapBridge::instance().begin(
            h->snap(), xapp->display(), ref, geo))
    {
        flamewm::engine::icewm::SnapBridge::instance().setWorkspaceDwellCallback(
            [this](flamewm::platform::snap::SnapTarget target) {
                auto restore = flamewm::engine::icewm::SnapBridge::instance().dragAway();
                if (restore.ok()) {
                    auto g = restore.value();
                    setCurrentGeometryOuter(YRect(g.x, g.y,
                                                  (unsigned)g.w, (unsigned)g.h));
                }
                if (target == flamewm::platform::snap::SnapTarget::LeftHalf)
                    manager->switchToPrevWorkspace(true);
                else if (target == flamewm::platform::snap::SnapTarget::RightHalf)
                    manager->switchToNextWorkspace(true);
            });
        snapBridgeToken_ = (void*)1;
    }
}
void YFrameWindow::snapBridgeMoveMotion(const XMotionEvent& m) {
    if (!snapBridgeToken_) return;
    auto* h = flamewm::engine::icewm::Bridge::instance().host();
    if (!h || !h->snap()) return;
    Window w = client() ? client()->handle() : handle();
    flamewm::api::WindowRef ref = flamewm::engine::icewm::WindowAdapter::currentRef(
        static_cast<uint64_t>(static_cast<unsigned long>(w)));
    if (!ref.valid()) return;
    int screen = desktop->getScreenForRect(m.x_root, m.y_root, 1, 1);
    YRect outRect = desktop->getScreenGeometry(screen);
    flamewm::api::Rect out(outRect.x(), outRect.y(), (int)outRect.width(), (int)outRect.height());
    (void)ref;
    flamewm::engine::icewm::SnapBridge::instance().motion(
        flamewm::api::Point(m.x_root, m.y_root), out);
}
void YFrameWindow::snapBridgeMoveEnd(bool cancel) {
    if (!snapBridgeToken_) return;
    auto* h = flamewm::engine::icewm::Bridge::instance().host();
    if (h && h->snap()) {
        Window w = client() ? client()->handle() : handle();
        flamewm::api::WindowRef ref = flamewm::engine::icewm::WindowAdapter::currentRef(
            static_cast<uint64_t>(static_cast<unsigned long>(w)));
        if (!ref.valid() || ref != flamewm::engine::icewm::SnapBridge::instance().window()) {
            snapBridgeToken_ = nullptr;
            flamewm::engine::icewm::SnapBridge::instance().cancel();
            return;
        }
        if (!cancel) {
            auto res = flamewm::engine::icewm::SnapBridge::instance().end(false);
            if (res.ok()) {
                auto g = res.value();
                setCurrentGeometryOuter(YRect(g.x, g.y, (unsigned)g.w, (unsigned)g.h));
            }
            else {
                auto restore = h->snap()->dragAway(ref);
                if (restore.ok()) {
                    auto g = restore.value();
                    setCurrentGeometryOuter(YRect(g.x, g.y,
                                                  (unsigned)g.w, (unsigned)g.h));
        ...
        } else flamewm::engine::icewm::SnapBridge::instance().end(true);
    }
    snapBridgeToken_ = nullptr;
    snapBridgeHidePreview();
}
void YFrameWindow::snapBridgeHidePreview() {
    flamewm::engine::icewm::SnapBridge::instance().hidePreview();
}
#endif
// FLAMEWM-BRIDGE-HOOK-END(FW-SNAP-02)
