...
use rustwebrender_core::{Color, Rect};
...
type CreatePictureFn = unsafe extern "C" fn(*mut Display, Drawable, *mut XRenderPictFormat, c_ulong, *const c_void) -> Picture;
type CreateSolidFillFn = unsafe extern "C" fn(*mut Display, *const XRenderColor) -> Picture;
type CompositeFn = unsafe extern "C" fn(*mut Display, c_int, Picture, Picture, Picture, c_int, c_int, c_int, c_int, c_int, c_int, u32, u32);
...
impl XRenderBackend {
    pub unsafe fn new(display: *mut Display, drawable: Drawable, visual: *mut Visual) -> Result<Self, String> {
        ...
        Ok(Self { display, destination, format, api, _library: library, solids: HashMap::new() })
    }

    pub unsafe fn set_drawable(&mut self, drawable: Drawable) -> Result<(), String> {
        let replacement = (self.api.create_picture)(self.display, drawable, self.format, 0, std::ptr::null());
        if replacement == 0 {
    ...
    pub unsafe fn fill_rounded_rect(&mut self, rect: Rect, radius: f32, color: Color) -> Result<(), String> {
    ...
    pub unsafe fn stroke_rounded_rect(
        ...
        for row in 0..height {
            ...
            if row < thickness || row >= height.saturating_sub(thickness) || width <= thickness * 2 || height <= thickness * 2 {
                self.fill_rect(x + outer_left as i32, y + row as i32, outer_right - outer_left, 1, color)?;
                continue;
            }
            ...
            if inner_left > outer_left {
                self.fill_rect(x + outer_left as i32, y + row as i32, inner_left - outer_left, 1, color)?;
            }
            if outer_right > inner_right {
                self.fill_rect(x + inner_right as i32, y + row as i32, outer_right - inner_right, 1, color)?;
            }
        }
        Ok(())
    }

    unsafe fn fill_rect(&mut self, x: i32, y: i32, width: u32, height: u32, color: Color) -> Result<(), String> {
