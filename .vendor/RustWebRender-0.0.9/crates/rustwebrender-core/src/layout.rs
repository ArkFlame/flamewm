...
impl<'a> Engine<'a> {
    ...
    fn measure_node(&self, index: u32, avail_w: f32, avail_h: f32) -> (f32, f32) {
        ...
        let inner_w = explicit_w.unwrap_or(avail_w).max(style.padding.horizontal()) - style.padding.horizontal();
        let inner_h = explicit_h.unwrap_or(avail_h).max(style.padding.vertical()) - style.padding.vertical();
    ...
    fn layout_node(
        ...
        if node.kind == NodeKind::Text || node.kind == NodeKind::Image {
            ...
            let width = forced_w.unwrap_or_else(|| style.width.resolve(avail_w).unwrap_or(measured_w));
            let height = forced_h.unwrap_or_else(|| style.height.resolve(avail_h).unwrap_or(measured_h));
            ...
            self.boxes[index as usize].rect = Rect { x, y, width, height };
            return (width, height);
        }
        ...
        self.boxes[index as usize].rect = Rect { x, y, width, height: provisional_height };
        let children = self.children(index);
        let consumed = match style.display {
            Display::None => (0.0, 0.0),
            Display::Block => self.layout_block(&children, content_x, content_y, content_w, content_h, style.gap),
            Display::Flex => self.layout_flex(&children, content_x, content_y, content_w, content_h, &style),
        };

        if height.is_none() {
            height = Some(consumed.1 + style.padding.vertical());
        }
        let final_height = clamp_length(height.unwrap_or(0.0), style.min_height, style.max_height, avail_h);
        self.boxes[index as usize].rect.height = final_height;

        self.layout_absolute_children(index, content_x, content_y, content_w, (final_height - style.padding.vertical()).max(0.0));
        (width, final_height)
    }
    ...
    fn layout_flex(
        ...
        let (mut cursor, distributed_gap) = match style.justify_content {
            ...
            JustifyContent::SpaceBetween if flow.len() > 1 => (0.0, style.gap + slack / (flow.len() - 1) as f32),
        ...
        for (slot, (child, base_main, measured)) in bases.into_iter().enumerate() {
            ...
            let margin_main_before = if is_row { child_style.margin.left } else { child_style.margin.top };
            let margin_main_after = if is_row { child_style.margin.right } else { child_style.margin.bottom };
            let margin_cross_before = if is_row { child_style.margin.top } else { child_style.margin.left };
            let margin_cross_after = if is_row { child_style.margin.bottom } else { child_style.margin.right };
            cursor += margin_main_before;

            let explicit_cross = if is_row { child_style.height.resolve(height) } else { child_style.width.resolve(width) };
            let measured_cross = if is_row { measured.1 } else { measured.0 };
            let available_cross = (cross_available - margin_cross_before - margin_cross_after).max(0.0);
            ...
            let cross_offset = match style.align_items {
                ...
                AlignItems::Center => margin_cross_before + (available_cross - cross).max(0.0) / 2.0,
            ...
            let (actual_w, actual_h) = self.layout_node(child, child_x, child_y, width, height, forced_w, forced_h);
    ...
    fn layout_absolute_children(&mut self, parent: u32, x: f32, y: f32, width: f32, height: f32) {
        for child in self.children(parent) {
            ...
            let forced_h = style.height.resolve(height).or_else(|| match (top, bottom) {
            ...
            let child_x = x + left.unwrap_or_else(|| right.map(|value| width - value - child_w).unwrap_or(0.0));
            let child_y = y + top.unwrap_or_else(|| bottom.map(|value| height - value - child_h).unwrap_or(0.0));
            self.layout_node(child, child_x, child_y, width, height, Some(child_w), Some(child_h));
        }
    }
}
...
mod tests {
    ...
    fn flex_grow_fills_available_width() {
        ...
        let document = CompiledDocument {
            ...
            assets: Vec::new(),
            nodes: vec![
                CompiledNode { kind: NodeKind::Element, parent: None, first_child: Some(1), next_sibling: None, id: String::new(), action: String::new(), text: String::new(), image: None, style: root_style, hover_style: None, active_style: None },
                CompiledNode { kind: NodeKind::Element, parent: Some(0), first_child: Some(3), next_sibling: Some(2), id: String::new(), action: String::new(), text: String::new(), image: None, style: left, hover_style: None, active_style: None },
                CompiledNode { kind: NodeKind::Element, parent: Some(0), first_child: None, next_sibling: None, id: String::new(), action: String::new(), text: String::new(), image: None, style: right, hover_style: None, active_style: None },
