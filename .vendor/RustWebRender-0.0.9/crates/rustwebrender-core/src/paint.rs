...
pub enum PaintCommand {
    ...
    Image { rect: Rect, asset: u16 },
    Text { x: f32, y: f32, color: Color, size: f32, weight: u16, text: String },
}
...
pub fn build_paint_commands(
    ...
    for index in indices {
        ...
        let rect = layout.boxes.get(index).map(|entry| entry.rect).unwrap_or_default();
