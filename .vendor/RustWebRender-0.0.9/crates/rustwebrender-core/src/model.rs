    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (index, asset) in self.assets.iter().enumerate() {
            ...
            let expected = asset
                ...
                .ok_or_else(|| format!("image asset {index} dimensions overflow"))? as usize;
        ...
        for (index, node) in self.nodes.iter().enumerate() {
            ...
            if let Some(asset) = node.image {
                if asset as usize >= self.assets.len() {
                    return Err(format!("node {index} references invalid image asset {asset}"));
                }
                if node.kind != NodeKind::Image {
                    return Err(format!("node {index} has image data but is not an image node"));
                }
            }
...
impl RuntimeDocument {
    ...
    pub fn set_background_color(&mut self, id: &str, color: Color) -> Result<(), String> {
        let index = self
            .node_by_id(id)
            .ok_or_else(|| format!("no node with id '{id}'"))?;
        self.style_overrides.entry(index).or_default().background = Some(ColorValue::Literal(color));
        Ok(())
    }
    ...
    pub fn set_border_color(&mut self, id: &str, color: Color) -> Result<(), String> {
        let index = self
            .node_by_id(id)
            .ok_or_else(|| format!("no node with id '{id}'"))?;
        self.style_overrides.entry(index).or_default().border_color = Some(ColorValue::Literal(color));
        Ok(())
    }
    ...
    pub fn set_flex_direction(&mut self, id: &str, direction: FlexDirection) -> Result<(), String> {
        let index = self
            .node_by_id(id)
            .ok_or_else(|| format!("no node with id '{id}'"))?;
        self.style_overrides.entry(index).or_default().flex_direction = Some(direction);
        Ok(())
    }
    ...
    pub fn set_ui_font(&mut self, family: impl Into<String>, size_offset: f32, bold: bool) -> Result<(), String> {
    ...
    pub fn runtime_style(&self, index: u32, interaction: InteractionState) -> Style {
        ...
        if let Some(geometry) = self.geometry_override(index) {
            if let Some(left) = geometry.left { style.left = Length::Px(left); }
            if let Some(top) = geometry.top { style.top = Length::Px(top); }
            if let Some(width) = geometry.width { style.width = Length::Px(width); }
            if let Some(height) = geometry.height { style.height = Length::Px(height); }
        }
        if let Some(override_style) = self.style_overrides.get(&index) {
            if let Some(background) = override_style.background { style.background = background; }
            if let Some(border_color) = override_style.border_color { style.border_color = border_color; }
            if let Some(opacity) = override_style.opacity { style.opacity = opacity; }
            if let Some(direction) = override_style.flex_direction { style.flex_direction = direction; }
            if let Some(font_size) = override_style.font_size { style.font_size = font_size; }
            if let Some(font_weight) = override_style.font_weight { style.font_weight = font_weight; }
        }
