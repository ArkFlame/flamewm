...
use flamewm_render_core::{
    Color, CursorKind, ImageAsset, InteractionState, LayoutEngine, LayoutResult, PaintCommand,
    Rect, RuntimeDocument, build_paint_commands,
};
        ...
        loop {
            ...
            match event.type_ {
                ...
                CLIENT_MESSAGE => {
                    ...
                    if self.wm_delete != 0
                        && client.format == 32
                        && client.data.l[0] as u64 == self.wm_delete
                    {
                        return Ok(());
                    }
                }
...
unsafe fn set_window_role(display: *mut Display, window: Window, role: X11WindowRole) {
    ...
    let property = XInternAtom(display, property_name.as_ptr(), 0);
    let role_atom = XInternAtom(display, role_name.as_ptr(), 0);
