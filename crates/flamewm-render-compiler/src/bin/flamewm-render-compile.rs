...
fn main() {
    if let Err(error) = run() {
        eprintln!("flamewm-render-compile: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let input = input.ok_or("missing HTML input; run flamewm-render-compile --help")?;
...
fn print_help() {
    println!("FlameWM Render 0.0.9 build-time compiler");
    println!("usage: flamewm-render-compile [--css file.css] [--allow-unsupported] [-o output.rwr] input.html");
}
